...
export const getList = (token,search) => {
    return async (dispatch) => {
        ...
        if(response.ok) {
            ...
            dispatch(fetchListFailed("Fetching shopping information failed. Server responded with a status "
...
export const add = (token,item) => {
    return async (dispatch) => {
        ...
        if(response.ok) {
            ...
            if(response.status === 403) {
                dispatch(logoutFailed("Your session has expider. Logging you out"));
                return;
            }
            dispatch(addItemFailed("Failed to add new item. Server responded with a status "+response.
...
export const remove = (token,id) => {
    return async (dispatch) => {
        ...
        if(response.ok) {
            ...
            if(response.status === 403) {
                dispatch(logoutFailed("YOur session has expired. Loggin you out."))
                return;
            }
            dispatch(removeItemFailed("Removing item failed. Server responded with a status"+response.
            status+" "+response.statusText));
        }
    }
}
...
export const edit = (token,item) => {
    return async (dispatch) => {
        ...
        dispatch(stopLoading());
        if(!response) {
            dispatch(editItemFailed("Server never responsed. Try again later"));
            return;
        }
        if(response.ok) {
            ...
            dispatch(editItemFailed("Failed to edit item. Server responded with a status"+response.
            status+" "+response.statusText));
        }
    }
}
